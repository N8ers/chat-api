...
async function seed () {
  ...
  const conversation_members = await knex('conversation_members').insert([
    ...
    { user_id: users[2].id, conversation_id: conversations[1].id }
  ]).returning('*')
  ...
  const messages = await knex('messages').insert([
    ...
    { user_id: users[2].id, conversation_id: conversations[1].id, content: 'Sup bro'      },
    { user_id: users[3].id, conversation_id: conversations[1].id, content: 'Hi, Joe'      },
  ]).returning('*')
